    ...
    <div class="space-y-5">
        ...
        <Card>
            ...
            <div class="grid xl:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-5">
                <div class="price-table bg-opacity-[0.16] dark:bg-opacity-[0.36] rounded-[6px] p-6 text-slate-900 dark:text-white relative overflow-hidden z-[1]"
                    :class="item.bg" v-for="(item, i) in tables" :key="i">
                    <div v-if="item.ribon"
                    ...
                    <header class="mb-6">
                        ...
                        <!-- <div class="space-x-4 relative flex items-center mb-5 rtl:space-x-reverse">
                            <span class="text-[32px] leading-10 font-medium" v-if="check">
                                {{ item.price_Yearly }} </span><span class="text-[32px] leading-10 font-medium"
                                v-if="!check">
                                {{ item.price_Monthly }}
                            </span>
                        </div> -->
                    ...
                    <div class="price-body space-y-8">
                        <p class="text-sm leading-5 text-slate-600 dark:text-slate-300">
                            {{item.limit}} <br/>{{ item.charge }}
                        </p>
                        <div>
                            <Button :text="item.button" btnClass="btn-outline-dark dark:border-slate-400 w-full" @click="item.doit()" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            tables: [
                {
                    ...
                    price_Yearly: "$96",
                    price_Monthly: "$26",
                    limit: "Deposit Limit($10.00 - $100,000.00)",
                    charge: "Deposit Charge($0.00 + 2.00 %)",
                    button: "Deposit Now",
                    bg: "bg-warning-500",
                    doit: () => {
                        console.log("here")
