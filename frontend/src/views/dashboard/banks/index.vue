<template>
    <div>
        <Card noborder>
            <div class="md:flex pb-6 items-center">
                <h6 class="flex-1 md:mb-0 mb-3">Banks</h6>
            ...
            <div class="-mx-6">
                <template v-if="banks">
                    <vue-good-table :columns="columns" styleClass=" vgt-table  centered " :rows="banks"
                        :sort-options="{
                            enabled: false,
                        }" :pagination-options="{
    enabled: true,
    ...
    externalQuery: searchTerm,
                        ...
                        <template v-slot:table-row="props">
                            ...
                            <span v-if="props.column.field == 'charge_in_percentage'" class="font-medium">
                                <span v-if="props.row.charge_in_percentage">
                                    {{ props.row.charge_in_percentage + "%" }}
                                </span>
                            </span>
                            <span v-if="props.column.field == 'minimum_transfer_amount'" class="font-medium">
                                {{ props.row.bank_currency +" " + parseFloat(props.row.minimum_transfer_amount).toLocaleString("en-US") }}
                            </span>
                            <span v-if="props.column.field == 'maximum_transfer_amount'" class="font-medium">
                                {{ props.row.bank_currency +" " + parseFloat(props.row.maximum_transfer_amount).toLocaleString("en-US") }}
                            </span>
                        ...
                        <template #pagination-bottom="props">
                            <div class="py-4 px-3">
                                <Pagination :total="banks.length" :current="current" :per-page="perpage"
...
export default {
        ...
        return {
            ...
            columns: [
                ...
                },
                {
                    ...
                    field: "minimum_transfer_amount",
                },
                {
                    label: "Maximum Amount",
                    field: "maximum_transfer_amount",
                },
    ...
    methods: {
        ...
        edit_plan(id) {
            this.$router.push({ name: "admin-edit-bank", params : {bank_id : id} })
        },
        approve_request(pay_id) {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/bank_activate`, formData, {
                headers: {
        ...
        cancel_request(pay_id) {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/bank_deactivate`, formData, {
                headers: {
                    "Authorization": "Bearer " + this.$store.authStore.user.token
                }
            }).then(function (response) {
        ...
        fetch_all() {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/list_banks`, {}, {
                headers: {
                    "Authorization": "Bearer " + this.$store.authStore.user.token
                }
            }).then(function (response) {
        ...
        fetch_approved() {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/list_active_bank`, {}, {
                headers: {
                    "Authorization": "Bearer " + this.$store.authStore.user.token
                }
            }).then(function (response) {
        ...
        fetch_pending() {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/list_not_active_bank`, {}, {
                headers: {
                    "Authorization": "Bearer " + this.$store.authStore.user.token
                }
            }).then(function (response) {
