                ...
                <Card title="Edit Bank">
                    ...
                    <form @submit.prevent="onSubmit" class="space-y-4" enctype="multipart/form-data">
                        ...
                        <div class="grid lg:grid-cols-3 md:grid-cols-3 grid-cols-1 gap-5">
                            ...
                            <div class="fromGroup relative" :class="`${bankCountryError ? 'has-error' : ''}  `">
                                ...
                                <select name="swift" class="input-control block w-full focus:outline-none h-[48px]"
                                    ...
                                    <option v-for="item in Countries" :key="item" :value="item.label">{{ item.label }}
                                    </option>
                                </select>
                            ...
                            <div class="fromGroup relative" :class="`${bankCurrencyError ? 'has-error' : ''}  `">
                                ...
                                <select name="swift" class="input-control block w-full focus:outline-none h-[48px]"
                                    ...
                                    <option v-for="item in currencies" :key="item" :value="item.name">{{ item.name }}
                                    </option>
                                </select>

                                <span v-if="bankCurrencyError" class="mt-2 text-danger-500 block text-sm">{{
                                    bankCurrencyError
                                }}</span>
                            </div>
                        </div>
                        ...
                        <div class="grid lg:grid-cols-3 md:grid-cols-3 grid-cols-1 gap-5">
                            ...
                            <InputGroup type="number" label="Charge In Percentage" placeholder="Charge In Percentage"
                                v-model="chargeInPercent" :error="chargeInPercentError" classInput="h-[48px]" append="%">
                            </InputGroup>
...
export default {
    ...
    setup() {
        ...
        const onSubmit = handleSubmit((values) => {
            ...
            toast.info("Updating Bank", {
                timeout: 5000,
            });

            const fromData = new FormData();
            fromData.append("id", route.params.bank_id);
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/update_bank`, fromData, {
                headers: {
        ...
        let id = route.params.bank_id;
        let data = axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/get_bank`, {
            bank_id: id,
        }, {
            headers: {
                "Authorization": "Bearer " + auth.user.token
            ...
            if (response.data?.status) {
                ...
                name.value = response.data.bank.name
                swissCode.value = response.data.bank.swift_code
                bankCountry.value = response.data.bank.bank_country
                bankCurrency.value = response.data.bank.bank_currency
                minimum.value = response.data.bank.minimum_transfer_amount
                maximum.value = response.data.bank.maximum_transfer_amount
                fixedCharge.value = response.data.bank.fixed_charge
                chargeInPercent.value = response.data.bank.charge_in_percentage
                description.value = response.data.bank.descriptions
                status.value = response.data.bank.status
