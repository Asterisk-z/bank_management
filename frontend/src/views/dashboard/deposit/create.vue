                    ...
                    <form @submit.prevent="onSubmit" class="space-y-4" enctype="multipart/form-data">
                        ...
                        <div class="grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1 gap-5">
                            <div class="fromGroup relative" :class="`${selecteUserError ? 'has-error' : ''}  `">
                                ...
                                <select name="swift" class="input-control block w-full focus:outline-none h-[48px]" v-model="selecteUser">
                                    <option value="">Select A User</option>
                                    <option v-for="user in users" :value="user.id" v-bind:key="user.id">{{ user.name +" | "+ user.email }}
                                    </option>
                                </select>

                                <span v-if="selecteUserError" class="mt-2 text-danger-500 block text-sm">{{ selecteUserError }}</span>
                            </div>
                            
                            <InputGroup type="text" label="Amount" placeholder="Amount" v-model="amount" :error="amountError" classInput="h-[48px]"  >
                                <template v-slot:prepend>
                                    
                                <div class="fromGroup relative" :class="`${selectedCurrencyError ? 'has-error' : ''}  `">
                                    <select name="swift" class="input-control block w-full focus:outline-none h-[48px]" v-model="selectedCurrency">
                                        <option value="">Select A Currency</option>
                                        <option v-for="currency in currencies" :value="currency.value" v-bind:key="currency.value">{{ currency.label }}
                                        </option>
                                    </select>
                                    <span v-if="selectedCurrencyError" class="mt-2 text-danger-500 block text-sm">{{ selectedCurrencyError }}</span>
                                </div>
                                </template>
                            </InputGroup>
                        </div>
...
export default {
    ...
    setup() {
        ...
        const onSubmit = handleSubmit((values) => {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/create_deposit`, fromData, {
                headers: {
                ...
                if (response.data?.status) {
                    ...
                    router.push({name : 'admin-deposit-history'});
