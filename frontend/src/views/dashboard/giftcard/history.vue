    ...
    <div>
        <Card noborder>
            <div class="md:flex pb-6 items-center">
                <h6 class="flex-1 md:mb-0 mb-3">All Gift Card</h6>
            ...
            <div class="-mx-6">
                <template v-if="gift_cards">
                    <vue-good-table :columns="columns" styleClass=" vgt-table  centered " :rows="gift_cards"
                        :sort-options="{
                            enabled: false,
                        }" :pagination-options="{
                            enabled: true,
                        ...
                        <template v-slot:table-row="props">
                            <span v-if="props.column.field == 'user'" class="flex flex-col items-center">
                                <template v-if="props.row.is_general == 'yes'" >
                                        {{ "All Users" }}
                                </template>
                                <template  v-if="props.row.is_general == 'no'">
                                    <span class="text-sm text-slate-600 dark:text-slate-300 capitalize font-medium">{{ props.row.name }}</span>
                                    <span class="text-sm text-slate-600 dark:text-slate-300 capitalize font-medium">{{ props.row.email }}</span>
                                </template>
                        ...
                        <template #pagination-bottom="props">
                            <div class="py-4 px-3">
                                <Pagination :total="gift_cards.length" :current="current" :per-page="perpage"
...
export default {
    ...
    methods: {
        ...
        approve_request(pay_id) {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/activate_giftcard`, formData, {
                headers: {
                ...
                if (response.data?.status) {
                    ...
                    $this.$router.push({ name: 'admin-all-gift-card' })
        ...
        cancel_request(pay_id) {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/cancel_giftcard`, formData, {
                headers: {
                    "Authorization": "Bearer " + this.$store.authStore.user.token
                }
            }).then(function (response) {
        ...
        fetch_all() {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/list_gift_card`, {}, {
                headers: {
                    "Authorization": "Bearer " + this.$store.authStore.user.token
                }
            }).then(function (response) {
        ...
        fetch_approved() {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/list_active_gift_card`, {}, {
                headers: {
                    "Authorization": "Bearer " + this.$store.authStore.user.token
                }
            }).then(function (response) {
        ...
        fetch_pending() {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/list_used_gift_card`, {}, {
                headers: {
                    "Authorization": "Bearer " + this.$store.authStore.user.token
                }
            }).then(function (response) {
        ...
        fetch_declined() {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/list_canceled_gift_card`, {}, {
                headers: {
                    "Authorization": "Bearer " + this.$store.authStore.user.token
                }
            }).then(function (response) {
