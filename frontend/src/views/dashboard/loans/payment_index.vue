<template>
    <div>
        <Card noborder>
            <div class="md:flex pb-6 items-center">
                <h6 class="flex-1 md:mb-0 mb-3">Loan Payments</h6>
            ...
            <div class="-mx-6">
                <template v-if="loanpayments">
                    <vue-good-table :columns="columns" styleClass=" vgt-table  centered " :rows="loanpayments"
                        :sort-options="{
                            enabled: false,
                        }" :pagination-options="{
    enabled: true,
    ...
    externalQuery: searchTerm,
                        ...
                        <template v-slot:table-row="props">
                            ...
                            <span v-if="props.column.field == 'loan_ref'" class="font-medium">
                                <!-- <span v-if="props.row.loan_ref"> -->
                                    {{ props.row.loan.loan_ref   }}
                                <!-- </span> -->
                            </span>
                            ...
                            <span v-if="props.column.field == 'amount_to_pay'" class="font-medium">
                                {{ props.row.loan.currency + " " + parseFloat(props.row.amount_to_pay).toLocaleString("en-US") }}
                            </span>
                            <span v-if="props.column.field == 'interest'" class="font-medium">
                                {{ props.row.loan.currency + " " + parseFloat(props.row.interest).toLocaleString("en-US") }}
                            </span>
                            <span v-if="props.column.field == 'late_penalties'" class="font-medium">
                                {{ props.row.loan.currency + " " + parseFloat(props.row.late_penalties).toLocaleString("en-US") }}
                            </span>
                            <span v-if="props.column.field == 'total_amount'" class="font-medium">
                                {{ props.row.loan.currency + " " +(parseFloat(props.row.interest)+ parseFloat(props.row.amount_to_pay)).toLocaleString("en-US") }}
                            </span>
                        ...
                        <template #pagination-bottom="props">
                            <div class="py-4 px-3">
                                <Pagination :total="loanpayments.length" :current="current" :per-page="perpage"
...
export default {
    ...
    methods: {
        ...
        create_loan() {
            this.$router.push({ name: "admin-create-loan-payment" })
        },
        edit_plan(id) {
        ...
        fetch_all() {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/list_loan_payments`, {}, {
                headers: {
                    "Authorization": "Bearer " + this.$store.authStore.user.token
                }
            }).then(function (response) {
                ...
                if (response.data?.status) {
                    ...
                    //     timeout: 4000,
                    // });
                    $this.loanpayments = response.data.loanpayments
                } else {
                    let message = response.data?.message[0];
                    toast.error(message, {
                        timeout: 4000,
