    ...
    <div>
        ...
        <div class="grid grid-cols-12 gap-5">
            <div class="lg:col-span-8 col-span-12">
                <!-- {{ users }} -->
                <Card title="Create Loan Product">
                    ...
                    <form @submit.prevent="onSubmit" class="space-y-4" enctype="multipart/form-data">
                        ...
                        <div class="grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1 gap-5">
                            <InputGroup type="number" label="Interest Rate Per Year" placeholder="Interest Rate Per Year" v-model="interestRate"
                                    :error="interestRateError" classInput="h-[48px]">
                            </InputGroup>
                            ...
                            <div class="fromGroup relative" :class="`${interestTypeError ? 'has-error' : ''}  `">
                                ...
                                <span v-if="interestTypeError" class="mt-2 text-danger-500 block text-sm">{{ interestTypeError
                                }}</span>
                            </div>
                        </div>
                        
                        <div class="grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1 gap-5">
                             <InputGroup type="number" label="Term" placeholder="Terms" v-model="terms"
                                    :error="termsError" classInput="h-[48px]">
                                <template v-slot:prepend>
                                    ...
                                    <div class="fromGroup relative" :class="`${termPeriodError ? 'has-error' : ''}  `">
                                        ...
                                        <span v-if="termPeriodError" class="mt-2 text-danger-500 block text-sm">{{ termPeriodError
                                        }}</span>
                                    </div>
                                </template>
                            </InputGroup>
...
export default {
    ...
    setup() {
        ...
        const onSubmit = handleSubmit((values) => {
            ...
            axios.post(`${import.meta.env.VITE_APP_API_URL}/admin/create_loan_product`, fromData, {
                headers: {
